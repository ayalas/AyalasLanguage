...
export default function LoginPage(): React.ReactElement {
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    try {
      if (on2FA) {
        const response = await axios.post<LoginResponse<User>>('/admin/api/auth/verify2fa', { verify2FAToken, code } as Verify2FARequest);

        completeLogin(response.data.user);
      }
      else {
        const response = await axios.post<LoginResponse<User>>('/admin/api/auth/login', { userName: email, password } as LoginRequest);
