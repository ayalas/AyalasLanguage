...
export const EXERCISE_TYPE_LOGIC: Record<ExerciseType | 0, ExcerciseTypeLogic> = {
    ...
    [EXERCISE_TYPES.FILL_IN_THE_BLANKS]: createLogic({
        ...
        ai_json_format: `content: [{First: string, Second: string, Translation: string}] where each item inside content is an exercise, First would be the sentence in ${PLACEHOLDERS.TARGET_LANGUAGE_PLACEHOLDER} with ____ for blanks, and Second would be the sentence in ${PLACEHOLDERS.TARGET_LANGUAGE_PLACEHOLDER} with the blanks replaced by the correct answers, and Translation would be the sentence translation to in ${PLACEHOLDERS.KNOWN_LANGUAGE_PLACEHOLDER}. Example of one exercise (in this case learning English from French and not ${PLACEHOLDERS.TARGET_LANGUAGE_PLACEHOLDER} from ${PLACEHOLDERS.KNOWN_LANGUAGE_PLACEHOLDER}): { First: "I am going ____ the store ____ my friend", Second: "I am going to the store with my friend", Translation: "Je vais au magasin avec mon ami" }`
    }, {
